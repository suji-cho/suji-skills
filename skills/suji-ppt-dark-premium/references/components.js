// suji-ppt-dark-premium — 컴포넌트 라이브러리 + 실행 예시
// 사용: cd <skill dir> && npm i pptxgenjs && node references/components.js  (→ ~/Desktop/dark-premium-sample.pptx)
// 신규 덱: 이 파일을 복사 → 슬라이드 본문만 교체. 배경 에셋은 ../assets 사용.
...
// ===== DARK TOKENS =====
const TXT="F4F5F7",SUB="9CA3AF",DIM="5C6470",DIM2="3F454F",ACC="A9B2FF",ACC2="8C95E8",CARD="0F1118",CARDB="2A2E3A",TRK="1C1F28",LINEC="23262F",NODEFILL="0F1118";

function logo(s){ s.addText([{text:"HANCOM",options:{color:"D7DAE0"}},{text:".",options:{color:ACC}}],{x:10.9,y:0.5,w:1.9,h:0.3,fontFace:F,fontSize:14,bold:true,align:"right",margin:0}); }
function foot(s,no){ s.addText("ODL Enterprise 가격 결정 요청 · 제안 · 대외비",{x:0.7,y:7.04,w:9,h:0.24,fontFace:F,fontSize:9,color:DIM2,margin:0}); s.addText(no,{x:12.2,y:7.02,w:0.5,h:0.26,fontFace:F,fontSize:11,bold:true,color:"4A5160",align:"right",margin:0}); }
...
function card(s,x,y,w,h,title){ s.addShape(RR,{x,y,w,h,fill:{color:CARD},line:{color:CARDB,width:1},rectRadius:0.09}); if(title) s.addText(title.toUpperCase(),{x:x+0.34,y:y+0.26,w:w-0.6,h:0.3,fontFace:F,fontSize:11,color:DIM,charSpacing:1.4,margin:0}); }
...
// waterfall: items=[[label,value,isAccent]], maxV
function waterfall(s,x,y,w,h,items,maxV){ const n=items.length,gap=0.18,bw=(w-gap*(n-1))/n; items.forEach((it,i)=>{ const bx=x+i*(bw+gap),bh=(h-0.5)*(it[1]/maxV); s.addText((it[2]==='neg'?'−$':'$')+Math.abs(it[1])+"K",{x:bx,y:y+(h-0.5-bh)-0.28,w:bw,h:0.26,fontFace:F,fontSize:13,bold:true,color:it[2]==='hi'?ACC:(it[2]==='neg'?SUB:TXT),align:"center",margin:0}); s.addShape(RR,{x:bx,y:y+(h-0.5-bh),w:bw,h:bh,fill:{color:it[2]==='hi'?ACC:(it[2]==='neg'?"3A3F4C":"4A5060")},line:{type:"none"},rectRadius:0.04}); s.addText(it[0],{x:bx,y:y+h-0.42,w:bw,h:0.3,fontFace:F,fontSize:11,color:SUB,align:"center",margin:0}); }); }
// price ladder: pts=[[price,frac,name,isMe]]
function ladder(s,x,y,w,pts){ s.addShape(LN,{x,y,w,h:0,line:{color:"2A2E3A",width:1}}); pts.forEach(p=>{ const cx=x+w*p[1],me=p[3]; s.addShape(OV,{x:cx-(me?0.13:0.07),y:y-(me?0.13:0.07),w:me?0.26:0.14,h:me?0.26:0.14,fill:{color:me?ACC:"6B7280"},line:{type:"none"}}); const above=(pts.indexOf(p)%2===0)||me; s.addText(p[0],{x:cx-1,y:above?y-0.62:y+0.28,w:2,h:0.26,fontFace:F,fontSize:me?16:13,bold:true,color:me?ACC:TXT,align:"center",margin:0}); s.addText(p[2],{x:cx-1,y:above?y-0.34:y+0.54,w:2,h:0.24,fontFace:F,fontSize:me?10:9.5,color:me?SUB:DIM,align:"center",margin:0}); }); }
// table: rows[0]=header. cols=[w...]
...
pres.writeFile({fileName:path.join(os.homedir(),"Desktop","dark-premium-sample.pptx")}).then(f=>console.log("SAVED:",f)).catch(e=>console.error(e));
