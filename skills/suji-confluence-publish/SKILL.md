...
description: ~/work/drafts 파일을 Confluence에 업로드할 때 사용. 제목 컨벤션(yyyymmdd), 500px 중앙 레이아웃, frontmatter 제거, 미확인 항목 체크를 자동 적용한다.
---

# /suji-confluence-publish

드래프트를 Confluence에 올릴 때 이 스킬을 사용한다.
목적은 하나: **모든 Confluence 페이지가 동일한 제목 형식, 레이아웃, 변환 규칙을 따르게 한다.**

## When to Trigger

- "Confluence에 올려줘", "페이지 배포해줘"
- ~/work/drafts/ 파일을 Confluence로 업로드할 때
...
|---|---|
| `20260313_해외보도자료_배포_효용성_분석.md` | `20260313 해외 보도자료 배포 효용성 분석` |
| `20260320_SaaS_구독_비용_정리.md` | `20260320 SaaS 구독 비용 정리` |

규칙:
- 로컬은 언더스코어(`_`), Confluence는 스페이스(` `)
- 날짜는 8자리 `yyyymmdd`
- 날짜 뒤 제목은 한글 자연어 띄어쓰기

---

## 2. 레이아웃

Confluence storage format(HTML) 또는 ADF 생성 시 필수:
...
### "Confluence에 올려줘" 요청 시

1. 대상 파일 확인 (`~/work/drafts/` 내 파일)
